...
async function askOpenAI(prompt) {
  ...
  try {
    const response = await axios.post('https://api.openai.com/v1/chat/completions', {
      model: "gpt-4o-mini",
      messages: [
        { role: "system", content: "Eres un experto contable que ayuda a conciliar transacciones de Jeeves con registros manuales de Excel." },
...
async function processExcel(db, filePath) {
  ...
  for (const sheetName of sheetNames) {
    ...
    for (const row of dataRows) {
      ...
      if (exactMatches.length === 1) {
        updateTxn(db, exactMatches[0].unique_id, expenseType, descCargo, folio, poId, 0);
      ...
      if (candidates.length > 0) {
        ...
        if (aiResult && aiResult.match_id && aiResult.confidence > 0.8) {
          updateTxn(db, aiResult.match_id, expenseType, descCargo, folio, poId, 0);
          totalStats.aiMatchedCount++;
...
function updateTxn(db, id, type, desc, folio, po, validation) {
  db.prepare(`
...
async function runHistoricalImport(db) {
  const historicosDir = path.resolve(__dirname, '../../Historicos');
  const files = [
    path.join(historicosDir, '______Jeeves - Prima______ enero.xlsx'),
    path.join(historicosDir, '______Jeeves - Prima______ febrero.xlsx'),
    path.join(historicosDir, '______Jeeves - Prima______ Marzo.xlsx')
  ];
  ...
  for (const file of files) {
    try {
      const stats = await processExcel(db, file);
      finalStats.updatedCount += stats.updatedCount;
      finalStats.aiMatchedCount += stats.aiMatchedCount;
      finalStats.pendingCount += stats.pendingCount;
...
module.exports = { runHistoricalImport };
